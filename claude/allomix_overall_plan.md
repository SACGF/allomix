...
- [ ] **Refactor: give the diagnostic scripts one shared "run a sample" entry point so they stop re-deriving package logic.** The `scripts/` plots each rebuild the genotype → classify → select-donor-hom → presence-test path by hand (`per_marker_rows` in `plot_host_presence_per_marker.py`, similar in `host_presence_manhattan.py` / `host_presence_markers_vcf.py`), and they reach into private helpers (`detect._is_artifact_marker`, `detect.ArtifactThresholds`) to mark filtered markers. This already bit us: `plot_host_presence_per_marker.py` had `use_sex_chroms=True` hardcoded, which diverged from the `monitor` default (False) and silently added chrX donor-hom markers back in, inflating per-sample counts (e.g. NDAD 102 vs the batch's 97) and re-introducing exactly the sex-chrom markers run10 excludes. Each new knob (sex chroms, artifact filter, per-site error table, future per-site dropout) has to be re-plumbed into every script and they drift. Proposed fix: expose a small public function in the package (e.g. `cli._run_single_sample` factored into a reusable `analyse_sample(...)` returning the classified markers + `HostPresenceResult` + the per-marker selection with artifact flags) and have the diagnostics consume that one path, so filters/sex/error-table handling is defined once. Promote whatever per-marker detail the plots need (the kept/filtered split, per-marker `y/n/h/direction`) onto a public surface so scripts don't import `_`-prefixed internals. Pairs naturally with the Step 12 per-marker-context refactor.
