...
For v2, in priority order:

- [ ] **Fix the joint-calling claim (correctness bug).** `methods.md` (the "For low-fraction donor detection ... should be joint-called" paragraph) and `discussion.md` ("admixture samples should be joint-called alongside donor and host samples") still describe the old single-phase model. This directly contradicts the tool's two-phase design (CLAUDE.md and `doc/joint_calling.md`): host/donor `GT` from GATK joint calling, but admix `AD` from forced `bcftools mpileup` at panel sites, because `HaplotypeCaller -ERC GVCF` strips minority ALT reads at hom-ref blocks (the exact low-fraction signal we need). `supplementary.md` is already correct. This must be fixed before colleagues read it.
- [ ] **Add a real-data demonstration section.** The paper is currently 100% in silico; the single biggest value-add for colleagues is real titrated-mixture data. The SRP434573 public run is already done (HiSeq 3000 MIP panel, ~1050 single-strand sites, two/three-person mixtures titrated 10% -> 0.5%, minor contributor mapped to HOST so it reads as a declining-chimerism series). Fold in the headline result with honest caveats (the ~0.2% co-pooled contamination floor, the REVIEW samples). Scaffolding is in `paper/public_data/SRP434573/`. Keep wetlab dilution-series validation (Step 11) as future work in the discussion, which already says the validation is "entirely in silico" and explains the overdispersion gap.
- [ ] **Embed the bias-stability figure** (`fig_bias_stability.png`, already generated by `generate_supp_synthetic.py` and wired into `Snakefile`) into `results.md`/`supplementary.md`. It validates the fixed-bias-per-marker assumption.
- [ ] **Add a no-overdispersion (binomial) baseline to the ablation (Fig S4).** Cheap, and it strengthens the paper's main quantitative finding (overdispersion, not depth, controls LoD at clinical coverage).
