...
The recipient clone routinely carries somatic copy-number changes, which the diploid VAF model ignores. Added a copy-number-aware mixture to `simulate.py` (`HostAberration`, `cn_weighted_vaf`, `assign_cnv_aberrations` for cnloh/deletion/gain, `blend_vcfs(host_aberrations=...)`) and a validation sweep (`paper/scripts/run_cnv_loh_validation.py` + `plot_cnv_loh.py`, Snakefile `cnv_loh_validation`/`cnv_loh_plot`). **Metric is the LoD of the minor component (CLSI EP17-A2, log donor-% axis like `plot_lod_curves`), swept in BOTH directions (recipient always carries the aberration): `host` = detect recipient relapse (minor) vs clean donor (early-warning); `donor` = detect donor (minor) vs CN-LoH recipient background (mixed chimerism). Findings — the two regimes differ sharply: relapse detection is ~unaffected (LoD flat ~0.5-1.6% across burden, robust no-op) because the aberration rides the minor component and the host-hom detector markers are CN-LoH-immune; but donor detection in mixed chimerism is wrecked (baseline ~0.5%/1.1% → >20% undetectable by 25% CN-LoH/deletion burden; deletion worst, gain mildest). The 3-SD outlier flag never refits, so it catches almost none of the aberrant markers.** Fix: a median/MAD robust refit, **shipped** as `estimate_single_donor_bb`/`estimate_multi_donor` `robust={off,auto,force}` (`_robust_refit`: iterative trim + full refit of f/rho/CI/LoD/GoF on survivors); CLI `--robust`/`--robust-k` on monitor/timeline (default `auto`), threaded via `analysis.analyse_sample`. Default-on but **gated** (engages only when first-pass outliers exceed `_robust_trigger`=max(3,ceil(0.03n)), so clean/validated samples are byte-identical), **floored** (`auto` keeps ≥`ROBUST_MIN_MARKERS`), and **flagged** (qc REVIEW + warning when drop fraction > `ROBUST_REVIEW_FRACTION`=0.15). In the donor-detection regime robust strongly recovers the gain LoD (4.4%→0.7%) and low-burden deletion/CN-LoH in unrelated donors (deletion 10%: >20%→6.6%); neutral at burden 0; cannot rescue ≥25% CN-LoH/deletion (flagged REVIEW instead). Relapse detection needs no help. The sweep records std+robust per cell in one pass; `plot_cnv_loh.py` overlays them. Still TODO: per-marker allele-balance QC flag (overlaps #14), host-depth CN profile, robust-loss MLE, corrupting the host reference GT. Detail: `claude/issue_13_cnv_loh_plan.md`.
