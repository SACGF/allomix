...
The recipient clone routinely carries somatic copy-number changes, which the diploid VAF model ignores. Added a copy-number-aware mixture to `simulate.py` (`HostAberration`, `cn_weighted_vaf`, `assign_cnv_aberrations` for cnloh/deletion/gain, `blend_vcfs(host_aberrations=...)`) and a validation sweep across all three kinds (`paper/scripts/run_cnv_loh_validation.py` + `plot_cnv_loh.py`, Snakefile `cnv_loh_validation`/`cnv_loh_plot`). **Metric is the donor LoD (CLSI EP17-A2, log donor-fraction axis, consistent with `run_lod_validation`): host carries the aberration, donor swept at low fractions. Findings: baseline donor LoD ~0.3% (unrel)/0.75% (sib); even 10% CN-LoH or deletion burden inflates it to ~16-20%, and ≥25% renders the donor undetectable (LoD >20%). Deletion worst (allele skew + DNA-mass + hits all genotypes), gain mildest (LoD only to a few %). The 3-SD outlier flag never refits, so it catches almost none of the aberrant markers.** Fix: a median/MAD robust refit, **shipped** as `estimate_single_donor_bb`/`estimate_multi_donor` `robust={off,auto,force}` (`_robust_refit`: iterative trim + full refit of f/rho/CI/LoD/GoF on survivors); CLI `--robust`/`--robust-k` on monitor/timeline (default `auto`), threaded via `analysis.analyse_sample`. Default-on but **gated** (engages only when first-pass outliers exceed `_robust_trigger`=max(3,ceil(0.03n)), so clean/validated samples are byte-identical), **floored** (`auto` keeps ≥`ROBUST_MIN_MARKERS`), and **flagged** (qc REVIEW + warning when drop fraction > `ROBUST_REVIEW_FRACTION`=0.15). Robust strongly recovers the gain LoD (4.5%→0.5%) and low-burden deletion/CN-LoH in unrelated donors (deletion 10%: >20%→6.9%); neutral at burden 0; cannot rescue ≥25% CN-LoH/deletion (flagged REVIEW instead). The sweep records std+robust per cell in one pass; `plot_cnv_loh.py` overlays them. Still TODO: per-marker allele-balance QC flag (overlaps #14), host-depth CN profile, robust-loss MLE, corrupting the host reference GT. Detail: `claude/issue_13_cnv_loh_plan.md`.
