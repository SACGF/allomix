...
### Follow-up (#5): investigate the ~0.22-point MLE floor on the semi-synthetic sub-0.5% ladder 🔲
The semi-synthetic SRP434573 sub-0.5% series (real reads, artificial mixing, no co-pooled contamination) showed the single-donor MLE host estimate sitting at a near-constant positive offset of about 0.22 percentage points: 0.1% host recovers at a median ~0.32%, rising in lockstep to ~0.74% at 0.5% (the presence test stayed specific at 0.1% and reached full detection by 0.4%, so it is the better sub-0.5% readout). Because these mixtures have no contamination floor, the offset is intrinsic to the estimator, not the dataset. Investigate the source: (a) the flat `error_rate` background over-attributing reads to host at the lowest fractions (would be addressed by the per-site error null above; re-run the ladder with `--error-table` and check the floor shrinks); (b) MLE boundary/grid behaviour near f=0; (c) residual semi-synthetic mixing bias (cross-check realised vs nominal on-target fraction). Quantify with the committed `paper/public_data/SRP434573/genotypes_synthetic` ladder; the offset is reported in Results. **[follow-up, investigate]**
