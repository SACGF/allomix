...
| `plot_chimerism_comparison.py` | Plot whole-blood NGS chimerism (with CIs) against flow-sorted lineage values, optionally overlaying a second run. |
...
sample  donor_pct  ci_lo  ci_hi  lob_pct  lod_pct  n_informative  n_used
        mean_depth  gof_pval  qc_pass  qc_warnings  <copied columns...>
```

- `lob_pct` / `lod_pct` — per-sample limit of blank / limit of detection.
- `qc_pass` — `PASS` / `FAIL`.
- `qc_warnings` — `; `-joined QC warnings (empty when clean). For a failing
  sample this names the cause, e.g. which input starved the informative markers.
...
### Example 2: two runs compared (and flag explicit-donor samples)
...
python scripts/plot_chimerism_comparison.py output/validation_run2/batch.tsv \
    ...
    --labels run2 run1 \
    --label-code \
    --explicit-donor AAAA,BBBB \
    --title "run1 (SID/tp0-donor) vs run2 (full panel/explicit donor ★)" \
    --output output/run1_vs_run2.png
```

The primary file is plotted on the right of each sample (run2), the
`--compare-tsv` file on the left (run1), so time reads left to right. Flow
lineage markers are drawn once (from the primary file). Each x-axis label shows
the patient code with that run's marker count and mean depth, e.g.
`REDACTED M:49 D:1356x`, coloured to match its run.

### Options

| Option | Default | Description |
|--------|---------|-------------|
| `batch_tsv` (positional) | — | Primary `batch.tsv` (e.g. run2). |
| `--compare-tsv` | none | Second `batch.tsv` to overlay (e.g. run1). |
| `--flow-column` | `Chimerism result TP2` | Column holding flow lineage strings like `CD45 100%; CD3 98%; CD13 88%`. |
| `--labels PRIMARY COMPARE` | `run2 run1` | Legend labels for the two runs. |
...
- **Blue (filled)** = primary run, **orange (open)** = compare run; the run name
  is carried by colour and the legend, not repeated in the labels.
