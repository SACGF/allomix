...
In silico validation demonstrated sub-percentage-point accuracy (RMSE {{ validation_summary.rmse_pct | dp(2) }}%) across the full range of donor fractions, using {{ validation_summary.n_informative | fmt('g') }} informative markers at {{ validation_summary.depth | fmt(',.0f') }}-fold depth. This level of accuracy is competitive with published performance metrics from commercial tools. Kakodkar et al. reported mean absolute errors of 0.3–1.5% for AlloSeq HCT across various donor fractions,[@Kakodkar2023alloseq] while Pedini et al. demonstrated comparable precision for the Devyser system at high sequencing depths.[@Pedini2021devyser]
