...
allomix separates the two things it needs from upstream calling, because they are best served by different tools. Host and donor genotypes (GT) come from GATK joint calling of the reference samples, which produces high-confidence germline calls. Admixture allele depths (AD) come from a separate forced pileup: bcftools mpileup at the panel sites followed by bcftools call constrained to the host/donor reference/alternative pair (-C alleles), so every panel site has a two-element AD (reference, alternative) regardless of whether the alternative allele was observed. Admixture samples are deliberately not taken from GATK. HaplotypeCaller in GVCF mode is a local-reassembly caller that records only reads supporting the called allele at homozygous-reference blocks, so the minority alternative reads that carry the low-fraction donor signal are discarded before joint calling and never reach the AD field. Forcing a pileup at the known panel sites retains them. The full rationale, including an empirical check on the rhAmpSeq panel showing zero alternative reads survive in joint-called admixture AD, is documented with the tool (doc/joint_calling.md).
...
To test allomix on real reads rather than simulated input alone, we used a public dataset of titrated DNA mixtures (SRA study SRP434573, BioProject PRJNA960854), deposited as companion data to a doctoral thesis (Chu, Huazhong University of Science and Technology, 2024) with no associated journal article. The dataset comprises seven unrelated individuals captured on an approximately 1,050-site molecular inversion probe (MIP) sample-identification panel and sequenced on an Illumina HiSeq 3000, with reads merged to single-end inserts (so each marker is read from a single strand). Pairwise two-person mixtures and one three-person mixture were prepared at known major:minor ratios, giving minor-contributor fractions of 10%, 5%, 2.5%, 1.25%, 1%, and 0.5%.

We assigned the minor (titrated) contributor to the host role and the major contributor to the donor role, so the monitored quantity is the host fraction and each dilution series mirrors a declining-chimerism (relapse) trajectory. Reads were aligned to hg38 with bwa-mem; because every MIP amplicon shares start and end coordinates, duplicate marking was not applied (it would discard almost all coverage). The capture-panel intervals were reconstructed from the aligned reads as high-depth amplicon clusters, and host/donor genotypes and admixture allele depths were generated with the two-phase pipeline described above (GATK joint calling for genotypes, forced bcftools pileup for admixture depths). allomix was run with default parameters and no panel-specific tuning; on this single-strand panel the host-presence strand-bias artifact filter auto-disables because almost every marker is read from one strand. The dataset covers the unrelated-donor case only and carries a co-pooled contamination floor characterised in Results.
