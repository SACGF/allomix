...
The 95% profile likelihood confidence intervals from the beta-binomial model showed coverage of {{ depth_1000.ci_coverage_pct }}--{{ depth_200.ci_coverage_pct }}% across depths (Table 1, Figure 3), below the nominal 95% level. The beta-binomial likelihood accounts for overdispersion via a shared concentration parameter rho, and when combined with per-marker bias correction, coverage improves further (from {{ bias_no_bias.ci_coverage_pct }}% to {{ bias_with_bias.ci_coverage_pct }}% at 2,000x depth). The remaining gap to nominal coverage reflects noise sources that a single shared rho cannot fully capture: locus dropout, heavy-tailed per-marker bias distributions where overdispersion varies across loci, and depth non-uniformity. Empirical CI recalibration from training data is a potential route to closing this gap.
